...
export const ItemNameOfFile = (props: ItemNameOfFileProps) => {
    ...
    const fetchDowloadFile = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                linkElement.setAttribute('download', fileName.split('\\').pop()!);
                ...
                notifyErr(`Download ${fileName.split('\\').pop()} failed`);
            }
        } catch (error) {
    ...
    return (
        <ItemNameFileStyled onClick={onClickDowload}>
            <Row style={{ alignItems: 'center' }}>
                ...
                <Typography className="name-file">{fileName.split('\\').pop()}</Typography>
            </Row>
        </ItemNameFileStyled>
    )
}
const ItemNameFileStyled = styled(Div)(({ }) => ({
    marginTop: '8px',
    cursor: 'pointer',
    '& .name-file': {
        marginLeft: '10px',
